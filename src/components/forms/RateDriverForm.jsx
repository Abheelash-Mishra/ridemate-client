...
const RateDriverForm = () => {
  ...
  const handleDriverIDChange = (e) => {
    const value = e.target.value.replace(/\D/g, "hi");
    setDriverID(value);
  };
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post(import.meta.env.VITE_BASE_URL + "/driver/rate", null, {
        params: {
          driverID: driverID,
          rating: rating,
        },
  ...
  return (
    <div
      className={
        "h-[28rem] flex flex-col justify-center items-center text-xl font-bold"
      }
    >
      <form
      ...
      >
        <label className="col-span-2 font-medium text-right">
          Enter Driver ID:
        </label>
        <input
          type="text"
